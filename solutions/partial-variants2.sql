-- Warianty progow CZESCIOWYCH z klucza CKE - Matura czerwiec 2021, zad. 6 (task6), plik 2.
--   task_3 - 1 pkt "za zaokraglenie do dwoch miejsc po przecinku" (wyniki + zaokraglenie,
--            bez sortowania malejacego)
--   task_5 - 1 pkt "za poprawne liczby koncertow poszczegolnych zespolow w weekendy"
...
-- task_3 BEGIN
-- Poprawne i zaokraglone srednie, ale posortowane alfabetycznie.
SELECT m.wojewodztwo, ROUND(COUNT(k.id) / COUNT(DISTINCT m.kod_miasta), 2) AS srednia
FROM miasta m
...
-- task_5 BEGIN
-- Poprawne liczby weekendowe, liczby dni powszednich zawyzone o 1.
SELECT z.nazwa,
       SUM(CASE WHEN DAYOFWEEK(k.data) IN (1, 7) THEN 1 ELSE 0 END) AS weekendy,
       SUM(CASE WHEN DAYOFWEEK(k.data) IN (1, 7) THEN 0 ELSE 1 END) + 1 AS dni_powszednie
FROM zespoly z
